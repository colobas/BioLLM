...
class DrugTask(BioTask):
    ...
    def scgpt_inference(self, gexpr_feature):
        adata = anndata.AnnData(X=gexpr_feature)
        scgpt_model = self.load_model()
    ...
    def run(self):
        ...
        # pretraining model
        gexpr_feature = self.scgpt_inference(gexpr_feature)
        ...
        if self.args.mode == 'train':
            ...
            # model.load_state_dict(torch.load(f'{self.args.save_path}/{self.args.model_used}/15.pth', map_location=self.device))
            # torch.save(model, f'{self.args.save_path}/{self.args.model_used}/best_scgpt_model.pt')
            model = torch.load(f'{self.args.save_path}/{self.args.model_used}/best_scgpt_model.pt')
...
if __name__ == "__main__":
    with tempfile.TemporaryDirectory() as tmp_dir:
        config_file = '../../config/drug/scgpt_drug.toml'
