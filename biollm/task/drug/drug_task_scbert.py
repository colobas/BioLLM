...
class DrugTask(BioTask):
    ...
    def scbert_inference(self, gexpr_feature):
        adata = anndata.AnnData(X=gexpr_feature)
        scbert_model = self.load_model()
    ...
    def run(self):
        ...
        # pretraining model
        gexpr_feature = self.scbert_inference(gexpr_feature)
        ...
        if self.args.mode == 'test':
            model.load_state_dict(torch.load(f'{self.args.save_path}/{self.args.model_used}/8.pth', map_location=self.device))
            # torch.save(model, f'{self.args.save_path}/{self.args.model_used}/best_scbert_model.pt')
            # model = torch.load(f'{self.args.save_path}/{self.args.model_used}/best_scbert_model.pt')
...
if __name__ == "__main__":
    with tempfile.TemporaryDirectory() as tmp_dir:
        config_file = '../../config/drug/scbert_drug.toml'
