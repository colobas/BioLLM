...
class DrugTask(BioTask):
    ...
    def scfoundation_inference(self, gexpr_feature):
        adata = anndata.AnnData(X=gexpr_feature)
        scfoundation_model = self.load_model()
    ...
    def run(self):
        ...
        # pretraining model
        gexpr_feature = self.scfoundation_inference(gexpr_feature)
        ...
        if self.args.mode == 'train':
            ...
            # torch.save(model, f'{self.args.save_path}/{self.args.model_used}/best_scfoundation_model.pt')
            model = torch.load(f'{self.args.save_path}/{self.args.model_used}/best_scfoundation_model.pt')
...
if __name__ == "__main__":
    with tempfile.TemporaryDirectory() as tmp_dir:
        config_file = '../../config/drug/scfoundation_drug.toml'
