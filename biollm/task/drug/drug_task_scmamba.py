...
class DrugTask(BioTask):
    ...
    def mamba_inference(self, gexpr_feature):
        adata = anndata.AnnData(X=gexpr_feature)
        mamba_model = self.load_model()
    ...
    def run(self):
        ...
        # pretraining model
        gexpr_feature = self.mamba_inference(gexpr_feature)
        ...
        if self.args.mode == 'test':
            ...
            # torch.save(model, f'{self.args.save_path}/{self.args.model_used}/best_mamba_model.pt')
            # model = torch.load(f'{self.args.save_path}/{self.args.model_used}/best_mamba_model.pt')
...
if __name__ == "__main__":
    with tempfile.TemporaryDirectory() as tmp_dir:
        config_file = '../../config/drug/scmamba_drug.toml'
